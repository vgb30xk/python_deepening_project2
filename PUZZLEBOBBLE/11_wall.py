...
def prepare_bubbles():
    ...
    if next_bubble:
        ...
        curr_bubble = create_bubble()  # 새버블 만들기
...
def remove_adjacent_bubbles(row_idx, col_idx, color):
    ...
    if len(visited) >= 4:
        remove_visited_bubble()
        remove_hanging_bubble()
...
def visit(row_idx, col_idx, color=None):
    ...
    if (
        row_idx < 0
        or row_idx >= MAP_ROW_COUNT
        or col_idx < 0
        or col_idx >= MAP_COLUMN_COUNT
    ):
        return
    # 현재 CEll의 색이 color와 같은지 확인
...
def remove_not_visited_bubbled():
    bubbles_to_remove = [
...
def remove_hanging_bubble():
    visited.clear()
    for col_idx in range(MAP_COLUMN_COUNT):
        if map[0][col_idx] != ".":
            visit(0, col_idx)
    remove_not_visited_bubbled()
